        ...
        return try {
            ...
            val response = when (searchType) {
                is ApiCallType.Category ->
                    productsApi.productsByCategory(searchType.categoryName, limit = 20, skip = 20 * (position - 1))

                ApiCallType.Default ->
                    productsApi.products(limit = 20, skip = 20 * (position - 1))

                is ApiCallType.Search ->
                    productsApi.search(searchType.searchParameter, limit = 20, skip = 20 * (position - 1))
            }

            val prevKey = if (position == 1) null else position - 1
            val nextKey =
                if (response.skip >= response.total || response.products.isEmpty()) null else position + 1
